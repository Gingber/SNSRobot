 ...
 *
 */
public class FacebookLikeRobot extends AjaxCrawl {
	
	private static DefaultHttpClient httpclient;
	private static String proxyHost = "";
	private static int proxyPort = 0;
	
	public FacebookLikeRobot(DefaultHttpClient httpclient) {		
	...
	public logonItem InitHttpclientAndConnection(Task task) {
		...
		if(lgitem != null) {
			FacebookLikeRobot  tpr = new FacebookLikeRobot(httpclient);
			return lgitem;
		} else {
			return null;
		}
	...
	public boolean doWork(Task task) {
		...
		if(lgitem != null) {
			...
			if(html != null && html.contains(task.getTargetString())) {
				LogSys.nodeLogger.debug("Successfully do like action: [" + task.getResultString() + "] "
				...
				LogSys.nodeLogger.debug("Failure do like action: [" + task.getResultString() + "] "
						+ "to [" + task.getTargetString() + "]");
