 ...
 *
 */
public class FacebookShareRobot extends AjaxCrawl {
	
	private static DefaultHttpClient httpclient;
	private static String proxyHost = "";
	private static int proxyPort = 0;
	
	public FacebookShareRobot(DefaultHttpClient httpclient) {		
	...
	public logonItem InitHttpclientAndConnection(Task task) {
		...
		if(lgitem != null) {
			FacebookShareRobot  fbsr = new FacebookShareRobot(httpclient);
			return lgitem;
		} else {
			return null;
		}
	...
	public boolean doWork(Task task) {
		...
		if(lgitem != null) {
			...
			if(html != null) {
				LogSys.nodeLogger.debug("Successfully do share action: [" + task.getResultString() + "] "
				...
				LogSys.nodeLogger.debug("Failure do share action: [" + task.getResultString() + "] "
						+ "to [" + task.getTargetString() + "]");
