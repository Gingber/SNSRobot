...
public abstract class AjaxCrawl {
	...
	public String openLink(final DefaultHttpClient httpclient, final Task task, final int count, WebOperationResult webres) {
		String WebPageContent = null;
		Future<String> future = service.submit(new Callable<String>() {
			public String call() throws Exception {
				try {
					return WebOperationAjax.openLink(httpclient, task, count);
				} catch (Exception ex) {
					ex.printStackTrace();
				}
				return null;
		...
		try{
			WebPageContent = (String) future.get(30000, TimeUnit.MILLISECONDS);			
