...
import com.iie.twitter.tools.TwitterLoginCookieStore;
...
public class AdvanceLoginManager {
	...
	private boolean getAvailableCookie(CookieItem item, int accountId){
		...
		try {
			...
			ResultSet rs=sta.executeQuery("select cookie,username from tw_account where status='using' and id = " + accountId);
			if(rs.next()){
	...
	private List<String[]> GetAvailableCount(int accountId){
		...
		try {
			sta = con.createStatement();
			ResultSet rs=sta.executeQuery("select username,userpassword from tw_account where id = " + accountId);
			while(rs.next()){
	...
	public boolean trylogin(int accountId){
		//
		//先把表锁起来
		//
	...
	private boolean markStatus(String username,String status){
		...
		java.sql.Statement sta;
		try {
			PreparedStatement pst=con.prepareStatement("update tw_account set status=? where username=?");
	...
	private boolean MaskAsNotAvailable(String username){
		...
		java.sql.Statement sta;
		try {
			PreparedStatement pst=con.prepareStatement("update tw_account set status='frozen', health=false where username=?");
	...
	private boolean SaveCookieToDB(String username,List<Cookie> cookie){
		...
		java.sql.Statement sta;
		try {
			PreparedStatement pst=con.prepareStatement("update tw_account set status='using',count=1,health=true,cookie=? where username=?");
	...
	private boolean checkLoginStatus(){
		...
		HttpGet httpget = new HttpGet("https://twitter.com/TwoNightStand/followers");
		try {
			HttpResponse response = httpclient.execute(httpget);
